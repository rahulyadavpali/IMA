...
class Lesson_model extends CI_Model
{
	...
	{
		$sql = $this->db->query("SELECT * FROM `lesson_planner_db`");
		return $sql->result_array();
	}

	public function getidResult($less_id)
	{
		$query= $this->db->query("SELECT * FROM `lesson_planner_db` WHERE stream = '".$less_id."' ");
	...
	public function saveResultData($resultData)
	{
		...
		$this->db->insert('lesson_planner_db', $resultinfo);
	...
	{
		$sql =  $this->db->query("DELETE FROM `lesson_planner_db` WHERE id = '".$r_id."' ");
	}
