...
class Entrance_model extends CI_Model
{
	...
	{
		$sql = $this->db->query("SELECT * FROM `entarncetest_db`");
		return $sql->result_array();
	}

	public function getidResult($less_id)
	{
		$query= $this->db->query("SELECT * FROM `entarncetest_db` WHERE stream = '".$less_id."' ");
	...
	public function saveResultData($resultData)
	{
		...
		$this->db->insert('entarncetest_db', $resultinfo);
	...
	{
		$sql =  $this->db->query("DELETE FROM `entarncetest_db` WHERE id = '".$r_id."' ");
	}
